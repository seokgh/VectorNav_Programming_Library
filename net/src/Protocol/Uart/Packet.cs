...
public class Packet
{
	...
	private const int MaximumWriteCommandSize = 384;
	...
	public bool IsValid
	{
		get
		{
			if (Length < 7)  // minumum binary packet is 7 bytes, minimum ASCII is 8 bytes
				return false;
			...
			case PacketType.Ascii:
				...
				if (Buffer[_start + Length - 6] == '*')
				{
					// Appears we have a 16-bit CRC packet.

					var packetCrc = VectorNav.Data.Util.HexStrToUInt16(Buffer, _start + Length - 5);

					var computedCrc = Crc16.Compute(Buffer, _start + 1, Length - 7);
			...
			default:
				...
				if(message.Contains(bootloaderSignature))
				{
					return true;
				}
	...
	/// <summary>
	/// Returns <c>true</c> if the packet is a bootloader message otherwise <c>false</c>.
	/// </summary>
	...
	/// The extracted value.
	/// </returns>
  public sbyte ExtractInt8()
	{
		EnsureCanExtract(sizeof(sbyte));

    var d = (sbyte)Buffer[_curExtractLoc];

    _curExtractLoc += sizeof(sbyte);
  ...
  /// <summary>
  /// Extracts a <c>vecDop</c> data type from a binary packet and advances
  /// the next extraction point appropriately.
  /// </summary>
  /// <returns>
  /// The extracted value.
  /// </returns>
  public vecDop ExtractDop() {
    EnsureCanExtract(sizeof(float) * 7);
	...
	/// The number of bytes written to the buffer.
	/// </returns>
	public static int GenCmdFirmwareUpdate(byte[] buffer, int index, ErrorDetection errorDetectionMode)
	{
		var cmd = "$VNFWU";
	...
	/// The formated packet.
	/// </returns>
	public static Packet GenCmdFirmwareUpdate(ErrorDetection errorDetectionMode)
	{
		var p = new Packet(MaximumWriteCommandSize);

		p.Length = GenCmdFirmwareUpdate(p.Buffer, 0, errorDetectionMode);

		return p;
	}

	/// <summary>
	/// Generates a command packet to write a firmware update record.
	/// </summary>
	/// <param name="buffer">
	/// Buffer to place the command packet into.
	/// </param>
	...
	/// </param>
	/// <param name="record">
	/// The record to write to the bootloader
	/// </param>
	/// <returns>
	/// The number of bytes written to the buffer.
	/// </returns>
    public static int GetCmdFirmwareUpdateRecord(byte[] buffer, int index, ErrorDetection errorDetectionMode, string record)
    {
		var cmd = string.Format("$VNBLD,{0}", record);
	...
	public static Packet GenCmdWriteFirmwareUpdateRecord(ErrorDetection errorDetectionMode, string record)
        {
		var p = new Packet(MaximumWriteCommandSize);

		p.Length = GetCmdFirmwareUpdateRecord(p.Buffer, 0, errorDetectionMode, record);
    ...
    private string bootloaderSignature = "VectorNav Bootloader";
    }
