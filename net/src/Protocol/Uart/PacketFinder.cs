...
public class PacketFinder
{
	...
	/// </param>
	public void ProcessReceivedData(byte[] data, int index, int length, bool bootloaderFilter=false)
	{
		...
		for (var i = 0; i < length; i++, _runningDataIndex++)
		{
			...
			if (cur == Packet.AsciiStartChar || (bootloaderFilter && (!_asciiOnDeck.CurrentlyBuildingAsciiPacket && cur == Packet.BootloaderVersionStartChar)))
			{
				_asciiOnDeck.Reset();
			...
			else if ((bootloaderFilter || (!bootloaderFilter && _asciiOnDeck.AsciiEndChar1Found)) && (_asciiOnDeck.CurrentlyBuildingAsciiPacket && cur == Packet.AsciiEndChar2))
			{ 
