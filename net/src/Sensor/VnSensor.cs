...
#region Message Objects
    ...
    public class VnSensor
    {
	    ...
	    private void SimplePortOnDataReceived(object sender, EventArgs eventArgs)
	    {
            ...
            //string debug = Encoding.UTF8.GetString(_readBuffer, 0, numOfBytesRead);

		    _packetFinder.ProcessReceivedData(_readBuffer, 0, numOfBytesRead, _filterBootloaderResponses);
	    }
	    ...
	    private void PacketFinderOnValidPacketFound(object sender, PacketFoundEventArgs validPacketFoundEventArgs)
	    {
		    ...
		    if(_filterBootloaderResponses && !packet.IsBootloader)
		    { 
			    _filterBootloaderResponses = false;
		    }
	    ...
	    {
            string debug = Encoding.UTF8.GetString(toSend, 0, length);
		    ...
		    while (true)
		    {
			    ...
			    while (responsesToProcess.Count > 0)
			    {
				    ...
				    _waitingForResponse = false;
					Thread.Sleep(1); // 1 ms sleep needed for working with tactical devices.
					return p;
			    }
	    ...
	    public void FirmwareUpdateMode(bool waitForReply)
	    {
		    var toSend = Packet.GenCmdFirmwareUpdate(SendErrorDetectionMode);
		...
		public void SwitchProcessors(VnProcessorType processor, string model, string firmware)
		{
			string switchCmd = "";

			if (model.Contains("VN-300"))
			{
				switch (processor)
				{
					case VnProcessorType.GPS:
						if ((model.Contains("SMD")) || (model.Contains("DEV")))
						{
							// VN300_SMD_GPS
							switchCmd = "$VNSPS,1,1,115200";
						}
						else if (model.Contains("CR"))
						{
							// VN300_CR_GPS
							switchCmd = "$VNDBS,3,1";
						}
			...
			else if ((model.Contains("VN-110")) || (model.Contains("VN-210"))|| (model.Contains("VN-310")))
			{
				switch (processor)
				{
					case VnProcessorType.NAV:
						switchCmd = "$VNSBL,0";
						break;
					case VnProcessorType.GPS:
						if ((model.Contains("VN-310")) || (model.Contains("VN-210E")))
						{
							switchCmd = "$VNSBL,1";
						}
						break;
					case VnProcessorType.IMU:
						switchCmd = "$VNSBL,2";
						break;
				}
			}

			if (switchCmd.Length == 0)
			{
				throw new InvalidOperationException();
			}

			// Send switch command
			int length = switchCmd.Length;

			byte[] toSend = FinalizeCommand(switchCmd, ErrorDetection.Crc16, out length);
			Packet response;

			TransactionNoFinalize(toSend, 0, length, true, out response, 6000, 6000);

			// Wait 2 seconds after switching processors
			Thread.Sleep(2000);
		}

		public void FirmwareUpdate(uint baudRate, string filename)
		{
			string record = "";
			...
			string bootloaderVersion = CalibrateBootloader();

			// Send each record from the firmware update file one at a time
			do
			{
				record = GetNextFirmwareUpdateRecord();
				if ((record != null) &&(record.Length != 0))
				{
					// Remove the ':' from the beginning of the line.
					WriteFirmwareUpdateRecord(record.TrimStart(':'), true);
				}
			...
			// Reset baudrate
			SetFirmwareUpdateBaudRate(_baudrate);
			...
			// Wait 2 seconds after the reset
			Thread.Sleep(2000);
		}

		public void SetFirmwareUpdateBaudRate(uint baudRate)
		{
			((SerialPort)_simplePort).ChangeBaudRate(baudRate);
		}

		public void WriteFirmwareUpdateRecord(string record, bool waitForReply)
		{
            Packet p = Packet.GenCmdWriteFirmwareUpdateRecord(ErrorDetection.Crc16, record);
            _filterBootloaderResponses = true;
            Transaction(p, true, 6000, 6000);
        }

        // Returns the bootloader version if successful
        public string CalibrateBootloader()
		{
			string toSend = "                ";
			...
			_filterBootloaderResponses = true;
            TransactionNoFinalize(buffer, 0, buffer.Length - 2, true, out response, 7000, 7000);

            return response.ToString();
		}

		// Return NULL if not record is found
		public string GetNextFirmwareUpdateRecord()
		{
			string record = _firmwareUpdateFile.ReadLine();

			return record;
		}

		public void OpenFirmwareUpdateFile(string filename)
		{
			_firmwareUpdateFile = new System.IO.StreamReader(filename);
		}

		public void CloseFirmwareUpdateFile()
		{
			_firmwareUpdateFile.Close();
		}

		public enum VnProcessorType
		{
			NAV,    // Navigation Processor (Main Processor)
			GPS,    // GPS Processor
			IMU     // IMU Processor
		}
	    ...
	    private System.IO.StreamReader _firmwareUpdateFile;
	    private bool _filterBootloaderResponses = false;
    }
